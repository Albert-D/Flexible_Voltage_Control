...
for i in range(agent_num):
    ...
    #policy_net_dict = torch.load(f'check_points/policy_net/2023-08-09/Step_200_Seed_12_a{i}.pth')
...
if senario == 0:
    ax.set_ylim([10, 12.6])
    ax.axhline(y=11.4, color='g', linestyle='--', label = 'Lower bound')
elif senario == 1:
    ax.set_ylim([11.4, 14])
    ax.axhline(y=12.6, color='g', linestyle='--', label = 'Upper bound')
else:
    ax.set_ylim([10, 14])
    ax.axhline(y=12.6, color='g', linestyle='--', label = 'Upper bound')
    ax.axhline(y=11.4, color='g', linestyle='--', label = 'Lower bound')
